	...
	while(true) {
		...
		if(!deviated) {
			...
			if(mTable.hasGroup((int) pGid)) {
			    ...
			    System.out.println("[mcast recv]: I am a member; Got message from " + src + " via " + incoming + " [" + pGid + "]: ");
			...
			if(downStream.isEmpty() && !mTable.hasGroup((int) pGid)) {
			    System.out.println("DEBUG [mcast recv]: Dead-end; Not a group member; Got message from group #" + pGid + " via " + incoming + ", pruning...");
			...
			if(downStream.isEmpty() && !mTable.hasGroup((int) pGid)) {
			    ...
			    System.out.println("DEBUG [mcast recv]: My neighbors wants to exclude themselves, so sending prune message upstream. All neighbors pruned from group #" + pGid + " [Source: " + src + "]");
			    byte[] pkt = MulticastPacket.formatRPFHeader(pGid, pSource, nLaunch, 0, true, false);
			    //System.arraycopy(buff, 20, pkt, 20, 236);
			    InetAddress uaddr = InetAddress.getByName(upstream);
			    DatagramPacket sendPacket = new DatagramPacket(pkt, pkt.length, uaddr, socket.getLocalPort());
