...
(defun run-tests (&key (multi-line t) (mode 'fail)  (prompt nil) (disp-counts t) nodals)
  (if (not multi-line)
  ...
  (let ((count-pass 0) (count-fail 0))
	...
	(if multi-line
      ...
      (let ((result (funcall matchfn (test-getpat test) (if nodals (convert-expr-to-tree (test-getexpr test)) (test-getexpr test)))))
	...
	(when (or (and (eq mode 'fail) 
	  ...
	  (if prompt
		...
		(if (equalp (intern (string-upcase  action)) 's)
